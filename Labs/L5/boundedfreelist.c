 ...
 */
Ticket *nonblockingGetTicket(void) {
	Ticket *answer;
...
/*
 * blocking function to get ticket from free list
 * returns pointer to ticket
 */
Ticket *blockingGetTicket(void) {
	Ticket *answer;
	pthread_mutex_lock(&lock);
	while (freeTickets == 0)
...
void *th_fxn(void *args) {
	...
	while(! shutdown) {
		...
		Ticket *ticket;
		if (arg->ifBlocking)
			ticket = blockingGetTicket();
		else
			ticket = nonblockingGetTicket();
		if (ticket == NULL)
...
int main(int argc, char *argv[]) {
	...
	for (i = 0; i < nthreads; i++) {
		myargs[i].id = i;
		myargs[i].ifBlocking = i%2; /* odd values of i block */
		//myargs[i].ifBlocking = 1;	/* all values of i block */
